...
class TestModuleStoreSerializer(TestDumpToNeo4jCommandBase):
    ...
    def _extract_relationship_pairs(relationships, relationship_type):
        ...
        relationship_pairs = [
            ...
            # Heads up: the py2neo API actually overrides the standard, builtin
            # behavior of type(...) on Relationsihp objects, such that it returns
            # a string indicating the "type", instead of a class object as expected.
            # Hence, the unidiomatic typecheck. (╯°□°)╯︵ ┻━┻
            for rel in relationships if type(rel) == relationship_type # pylint: disable=unidiomatic-typecheck
        ]
